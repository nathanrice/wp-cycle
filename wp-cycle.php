...
function wp_cycle_plugin_action_links($links) {
	$wp_cycle_settings_link = sprintf( '<a href="%s">%s</a>', admin_url( 'upload.php?page=wp-cycle' ), __('Settings', 'wp-cycle') );
...
function wp_cycle_images_admin() { ?>
	...
	<h2><?php _e('WP-Cycle Images', 'wp-cycle'); ?></h2>
...
function wp_cycle_settings_admin() { ?>
	...
	<p class="submit">
	<input type="submit" class="button-primary" value="<?php _e('Save Settings', 'wp-cycle') ?>" />
	</form>
	...
	<input type="submit" class="button" value="<?php _e('Reset Settings', 'wp-cycle') ?>" />
	</form>
	<!-- End Reset Option -->
	</p>
